...
namespace
{
  ...
  expected(note n, chord_type t, note slash_note)
  {
    return {chord_recognition_result(n, t, note_interval(n, slash_note))};
  }
...
{
  template <>
  struct StringMaker<chord_recognition_result>
  {
...
{
  REQUIRE(
      to_string(chord_recognition_result(note::C, chord_type::major, interval::root)) ==
      "C Major");

  REQUIRE(
      to_string(chord_recognition_result(note::D, chord_type::major7, interval::root)) ==
      "D Major7");
  REQUIRE(
      to_string(chord_recognition_result(note::E, chord_type::minor, interval::root)) ==
      "E Minor");
  REQUIRE(
      to_string(chord_recognition_result(note::F, chord_type::minor7, interval::root)) ==
      "F Minor7");
  REQUIRE(
      to_string(chord_recognition_result(
          note::G, chord_type::dominant, interval::root)) == "G Dominant");
  REQUIRE(
      to_string(chord_recognition_result(note::A, chord_type::sus2, interval::root)) ==
      "A Sus2");
  REQUIRE(
      to_string(chord_recognition_result(note::B, chord_type::sus4, interval::root)) ==
      "B Sus4");

  REQUIRE(
      to_string(chord_recognition_result(
        note::C, chord_type::major, interval::major_third)) == "C Major / E");
  REQUIRE(
      to_string(chord_recognition_result(
          note::D, chord_type::major7, interval::major_seventh)) == "D Major7 / C#");
}
